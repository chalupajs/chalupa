 ...
 */
export class IntegrationTestBuilderStrategyFactory {
	...
	create(): Constructor<IBuilderStrategy<IntegrationTestArrangement>> {
		const { communicationFacade } = this

		class IntegrationTestBuilderStrategy implements IBuilderStrategy<IntegrationTestArrangement> {
			build(intermediateService: IIntermediateService): Promise<IntegrationTestArrangement> {
				const orchestrator = intermediateService.bridge().useFacade(communicationFacade)
		...
		return IntegrationTestBuilderStrategy
	}
}
